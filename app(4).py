"""
最強糾察員 v4 ── 修正版
修正：IndexError / 白字 / 抽牌獨立大畫面 / 回合計數邏輯
"""
...
def init_game(names: List[str], mode: str, mode_val: int):
    ...
    return dict(
        ...
        turn=0,
        # phase: "draw_screen" | "draw_done" | "action" | "pending_remove" | "pending_pause"
        phase="draw_screen",
        over=False,
        mode=mode,
        mode_val=mode_val,
        ...
        showing_transition=True,   # 遊戲剛開始先顯示第一位玩家提示
        transition_to=0,
        last_drawn_card=None,      # 記錄剛抽到的牌
    )
...
def action_draw(gs):
    ...
    if gs["deck"]:
        c = gs["deck"].pop()
        p.hand.append(c)
        gs["last_drawn_card"] = len(p.hand) - 1   # 剛抽到的牌在手牌最後
...
.stApp {
    background: linear-gradient(150deg, #fdfbff 0%, #fff0fb 40%, #f0fbff 70%, #fffff0 100%);
...
.main-title {
    ...
    font-size: 2.6rem;
    text-align: center;
    background: linear-gradient(135deg, #FF6B6B, #FFB347, #4ECDC4, #A29BFE);
...
.sub-title {
    text-align: center; color: #888; font-size: .78rem;
...
.card-selected {
    ...
    box-shadow: 0 0 0 3px #FFD700, 0 12px 26px rgba(0,0,0,.18) !important;
...
.card-desc  { font-size: .62rem; font-weight: 600; color: #333 !important; }
...
.big-card-desc  { font-size: .85rem; font-weight: 700; color: #333 !important; margin-top: 4px; }
...
.plate-balanced {
    ...
    background: rgba(67,160,71,.06) !important;
...
@keyframes balGlow {
    ...
    50%      { box-shadow: 0 0 24px rgba(67,160,71,.55); }
}
...
@keyframes activeGlow {
    ...
    50%      { box-shadow: 0 0 0 4px #FFD700, 0 4px 20px rgba(255,215,0,.4); }
}
...
.event-item {
    ...
    font-weight: 700; font-size: .8rem; color: #5d4037 !important;
...
.transition-banner {
    ...
    background: linear-gradient(135deg, #fffde7, #fff8e1, #fce4ec);
...
.draw-banner {
    ...
    box-shadow: 0 8px 32px rgba(100,181,246,.3);
...
.draw-sub {
    font-size: 1rem; color: #444 !important; font-weight: 700;
    margin-bottom: 18px;
}
...
.rank-bar-wrap { flex: 1; background: #e0e0e0; border-radius: 4px; height: 7px; overflow: hidden; }
...
def render_ranking(players, ci, gs):
    ...
    if gs["mode"] == "score":
        st.markdown(f'<div style="font-size:.73rem;text-align:center;color:#888;margin-top:4px;">🏁 目標：{gs["mode_val"]} 分</div>', unsafe_allow_html=True)
    elif gs["mode"] == "rounds":
        done  = gs["round_count"]
        total = gs["mode_val"] * len(players)
        pct   = int(done / total * 100) if total else 0
        st.markdown(f'<div style="font-size:.73rem;text-align:center;color:#888;margin-top:4px;">🔁 回合進度 {done}/{total}</div>', unsafe_allow_html=True)
...
def page_setup():
    ...
    with col_r:
        ...
        for cat, info in FOOD_CATS.items():
            st.markdown(f'<div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid #f0f0f0;font-size:.82rem;color:#1a1a1a;"><span>{info["emoji"]} {cat}</span><span style="font-weight:800;">+{info["pts"]} 分</span></div>', unsafe_allow_html=True)
...
def page_transition():
    ...
    with c2:
        if gs["round_count"] > 0:
            st.markdown(f'<div style="text-align:center;color:#777;font-size:.92rem;font-weight:700;margin-bottom:12px;">✅ {prev.name} 的回合結束</div>', unsafe_allow_html=True)
...
def page_draw():
    ...
    with c2:
        ...
        st.markdown(f'<div style="text-align:center;font-size:.8rem;color:#888;margin-bottom:10px;">🃏 牌堆剩 {len(gs["deck"])} 張 ｜ {mode_lbl}</div>', unsafe_allow_html=True)
        ...
        if cur.hand:
            ...
            for i, card in enumerate(cur.hand):
                with hcols[i % n_cols]:
                    ...
                    # 只在 action 階段且非 pending 狀態顯示選擇按鈕
                    if phase == "action":
        ...
        with st.expander(f"{medals[ri]} {p.name}  ── {p.score} 分", expanded=(ri == 0)):
            ...
            with dc2:
                st.markdown(f"""<div style="background:{p.color['light']};border:2px solid {p.color['header']};border-radius:14px;padding:14px;text-align:center;">
                    ...
                    <div style="font-size:1.3rem;font-weight:900;color:{p.color['text']};border-top:1px solid #eee;margin-top:6px;padding-top:6px;">= {p.score} 分</div>
                </div>""", unsafe_allow_html=True)
...
def main():
    ...
    if phase == "draw_screen":
        ...
        # action / pending_remove / pending_pause → 行動頁
        page_action()
